...
int main()
{
  ...
  double h = 0.6766;                     // Scaled Hubble constant
  ...
  density_conversion = (M_sun_g/(kpc_cm*kpc_cm*kpc_cm))*(1./M_H_g)*((redshift+1.)*(redshift+1.)*(redshift+1.))*
    (h*h);
  delta_x = (24.4/h)*(1./(redshift+1.))*(kpc_cm); // Size of one grid cell in cm
  Delta = (4.*pi)/(delta_x*delta_x*delta_x); // Volume ratio for scaling

  z[0] = 0.;                                 // The grid increases by step size del_z and goes from 0 to 1                                                                                                         
  B[0] = 0.;                                 // The thermal source function in this example is constant                                                                                                            
  I_plus[0] = 1.;                            // The scaled bottom-up specific intensity; 1 --> 10^-17*Delta                                                                            
  I_minus[0] = pow(10.,-22.)/pow(10.,-17);          // The scaled top-down specific intensity; 10^-22*Delta/10^-17*Delta                                                                               
  J[0] = 0.5*(I_plus[0]+I_minus[0]);         // The average specific intensity 
  S[0] = epsilon*B[0] + (1.-epsilon)*J[0];   // The source function Ch.4 section 4.4.4, equation 4.37                                                                                                              
  density[0] = density[0]*density_conversion;
  col_dens[0] = density[0]*delta_x;
  del_tau[0] = col_dens[0]*sigma_nu;
  ...
  for (i=1;i<n;i++)
    {
      ...
      z[i] = j*del_z;
      B[i] = 0.;
      I_plus[i] = I_plus[0]*(Delta/pow(j,2.));
      I_minus[i] = I_minus[0];
      J[i] = 0.5*(I_plus[i]+I_minus[i]);
      S[i] = epsilon*B[i] + (1.-epsilon)*J[i];
      density[i] = density[i]*density_conversion;
      col_dens[i] = density[i]*delta_x;
      del_tau[i] = col_dens[i]*sigma_nu;
      //printf("del_tau %2.5f\n", del_tau[i]);
    }
  // Not 100% sure this is the right thing to do about this
  del_tau[n-1] = del_tau[n-2];
