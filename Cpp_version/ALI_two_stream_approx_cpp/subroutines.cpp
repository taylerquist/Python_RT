...
void RT::PrintIonization(FILE *fp)
{
  ...
  printf("Ionization (i,x,del_tau,QHII\n)");

  // Open the files in a special way
  QHII_outfile.open("QHII_out.txt", std::ios_base::app); // append instead of overwrite                                                                                                                               
  eps_outfile.open("eps_out.txt", std::ios_base::app); 
  dtau_outfile.open("del_tau_out.txt", std::ios_base::app); 

  for(int i=0;i<n;i++){
    //fprintf(fp,"%d\t%e\t%e\t%e\t%e\t%e\n",i,xg[i]/kpc_cm,QHII[i],eps_grid[i],alpha[i],del_tau[i]);
    fprintf(fp,"%d\t%e\t%e\t%e\n",i,xg[i]/kpc_cm,del_tau[i],QHII[i]); 
    QHII_outfile << QHII[i];
    QHII_outfile << '\n';
  ...
  // Helps organize the txt file when reading and plotting the data                                                                                                                                                   
  QHII_outfile << "break\n";
  eps_outfile << "break\n";
  dtau_outfile << "break\n";
...
void RT::InitializeGrid(void)
{
  ...
  //Create initial output files and open them
  std::ofstream QHII_outfile;
  std::ofstream eps_outfile;
  ...
  for(int i=0;i<n;i++)
  {
    ...
    //Append the value to its corresponding file
    QHII_outfile << QHII[i];
    QHII_outfile << '\n';
    eps_outfile << eps_grid[i];
    eps_outfile << '\n';
...
void RT::UpdateIntensity(void)
{
  ...
  //Open file to append
  J_outfile.open("J_out.txt", std::ios_base::app); // append instead of overwrite 
